...
Automated runs (`sortify-agent`) sort in the background, run subify per moved video, then register the results in Streama when `streama.enabled` is true in `agents/sortify-agent/config/agent.yaml`. When portal reporting is configured (`portal.enabled` plus `~/.config/catalog.ssh_alias/portal.env`), the agent also posts progress to the Support Portal and adds a Streama dashboard highlight (`Requested by {requestUser}`) so the request can reach **Available**.
