...
Automated runs (`sortify-agent`) sort in the background, run subify per moved video, then register the results in Streama when `streama.enabled` is true in `agents/sortify-agent/config/agent.yaml`. After each successful sort, the agent fetches missing English and Russian `.srt` files (OpenSubtitles), aligns them with `ffsubsync` (`--max-offset-seconds 12`), and keeps a file only when the sync gate passes (offset, optional score, cue count, duration coverage). Rejected downloads are deleted; the video still registers. **A `.srt` on disk is not in Streama until register / `addLocalFile`.** Register groups each video with sidecars from beside the file and from season `subs/` even when the episode sits in a nested release folder (`show/s01/release-name/episode.mp4` + `show/s01/subs/*.srt`). `addLocalFile` must send the path under `media/` (the Local Video Files tree). Resolved `/mnt/<uuid>/03_STORAGE/...` paths get a Streama **406**. The same acquire path can be queued from Request Update / Report Issue → **Add Subtitles** (`GET /agent/v1/jobs?kind=subtitle_acquire`). That remedia always `addLocalFile`s kept or already-on-disk sidecars onto the existing Streama video — it does not treat “file exists on disk” as already attached. Sortify finds library videos from Streama (`streamaMediaId`) when STORAGE paths no longer contain `tmdb{id}`. When portal reporting is configured (`portal.enabled` plus `~/.config/catalog.ssh_alias/portal.env`), the agent also posts `acquiring_subtitles` plus progress to the Support Portal and adds a Streama dashboard highlight (`Requested by {requestUser}`) so the request can reach **Available**.
...
Subs count as present when a matching `.srt` is beside the video or under any known subs layout in the source tree: `subs/`, `subs-{showname}/`, other `subs-*` folders, or `sNN/subs/` (including when the video is nested one level under `sNN/`, as in `s01/release-folder/episode.mp4`). Episode matching uses stem overlap or `SxxExx` in the path. Those same paths are what Streama register attaches; sidecars that are never discovered here never appear as subtitle tracks in the player.
